...
public class TrackerAnalyzeInfo {

	private Note notes[];
	private int nbNotes;
	
	public TrackerAnalyzeInfo() {
		notes = new Note[30];
		nbNotes = 0;
	}
	...
	public void readNote(String note) {
		...
		if (nbNotes < notes.length) {
			...
			if (temp.length() > 0 && timer > 0) {
				notes[nbNotes++] = new Note(temp, timer, 0);
				Main.sendNote(new Note(temp, timer, 0));
			}
		}
		else {
			return;
	...
	public void sortArray() {
	    ...
	    if (n > 1) {
		    for (int i = 0; i < n; i++) {
		        for (int j = 1; j < (n - i); j++) {
		            if (notes[j - 1].getTime() > notes[j].getTime()) {
		                tmp = notes[j - 1];
		                notes[j - 1] = notes[j];
		                notes[j] = tmp;
		            }
